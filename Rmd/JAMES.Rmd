...
We may now see the function return of `convert_bds_ind()` as (also in `R`)
...
Other types of output can also be downloaded.

## Requesting a growth chart

There are two server-side functions for generating a growth chart with the child's data: `draw_chart_bds()` and `draw_chart_ind()`. The difference is in the input. `draw_chart_bds()` takes the BDS-data, convert to `individual` and return the requested chart. 
...
The growth chart can be obtained as 

```{r eval=FALSE}
browseURL(extract_svg(resp2))
```

`draw_chart_ind()` loads the `individual` data stored on the server in a previous call, and return the requested chart. 

```{r}
# extract where the converted data are stored on server
loc <- extract_location(resp)

# request chart with chartcode for week 36
resp3 <- req_chart(location = loc, chartcode = "PJAAN36")
resp3
```
...
```{r eval=FALSE}
browseURL(extract_svg(resp3))
```

See <https://github.com/stefvanbuuren/james.client> for more detail.
