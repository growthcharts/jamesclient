...
```{bash}
curl http://54.93.78.215/ocpu/library/gateway/testdata/client3.json
```
...
```{bash, eval = FALSE}
curl http://54.93.78.215/ocpu/library/gateway/R/bds_to_individual \
...
```{r}
z <- gateway::get_result(resp, "stdout")[[1]]
content(z)
```
